...
exports.runTsInference = runTsInference;
const RULES = [
    // Malaria
...
const CONFIDENCE_RANK = { high: 3, medium: 2, possible: 1, none: 0 };
// ---------------------------------------------------------------------------
// Forward chaining engine — mirrors inference_engine.pl diagnose/3
// ---------------------------------------------------------------------------
function runTsInference(symptoms, duration) {
    ...
    for (const [disease, gatekeeper] of Object.entries(SYMPTOM_EXCLUSIONS)) {
        if (!symptomSet.has(gatekeeper))
            excluded.add(disease);
    }
    if (duration === 'less_than_2_weeks')
        excluded.add('HIV/AIDS');
    // Phase 2 — forward chaining: fire all rules whose conditions are met
    const firedRules = RULES.filter((rule) => !excluded.has(rule.disease) && rule.required.every((s) => symptomSet.has(s)));
    // Fallback
    ...
    const bestConfidence = Object.keys(CONFIDENCE_RANK).find((k) => CONFIDENCE_RANK[k] === bestRank);
...
//# sourceMappingURL=tsInferenceEngine.js.map